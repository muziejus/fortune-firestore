...
module.exports = Adapter =>
  class FirestoreAdapter extends Adapter {
    ...
    find(type, ids, options, meta) {
      ...
      options = options || {};
      const { Promise, db, recordTypes } = this;
      const { typeMap } = this.options;
      const collection = type in typeMap ? typeMap[type] : type;
      const dateKeys = Object.keys(recordTypes[type]).filter(
        key => recordTypes[type][key].type === Date
      );
        ...
        if (ids) {
          ...
          if (options.range) {
            for (const field of Object.keys(options.range)) {
              if (options.range[field][0] === null) {
                options.range[field][0] = 1;
              }
              if (options.range[field][1] === null) {
                options.range[field][1] = 1;
              }
              collectionRef = collectionRef
                .where(field, ">=", options.range[field][0])
                .where(field, "<=", options.range[field][1]);
            }
          }
