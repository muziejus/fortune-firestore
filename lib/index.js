...
module.exports = Adapter =>
  class FirestoreAdapter extends Adapter {
    ...
    find(type, ids, options) {
      ...
      const findDocuments = async () => {
        ...
        for (let doc of results) {
          if (doc) {
            doc = this._convertTimestampFieldsToDates(doc, dateKeys);
            doc = this._convertStringFieldsToBuffers(doc, bufferKeys, fields);
          }
        }
        ...
        const promises = records.map(record => {
          ...
          return reference
            .then(doc => {
              if (doc.exists) {
                let data = doc.data();
                data = this._convertTimestampFieldsToDates(data, dateKeys);
                return this._convertStringFieldsToBuffers(
                  data,
                  bufferKeys,
                  fields
                );
    ...
    _convertTimestampFieldsToDates(doc, dateKeys) {
    ...
    _convertStringFieldsToBuffers(doc, bufferKeys, fields) {
      bufferKeys.forEach(key => {
        if (doc[key]) {
          if (fields[key].isArray) {
