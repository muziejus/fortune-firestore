/* eslint max-depth: 1, no-eq-null: 1, eqeqeq: 1, no-case-declarations: 1, no-void: 1, unicorn/explicit-length-check: 1 */
"use strict";
...
function outputRecord(type, record) {
  ...
  for (const field of Object.getOwnPropertyNames(fields)) {
    const value =
      field in record ? record[field] : fields[field][isArrayKey] ? [] : null;
...
function mapValues(object, map) {
  return Object.keys(object).reduce(
    (clone, key) => Object.assign(clone, { [key]: map(object[key], key) }),
    {}
  );
}
...
function generateRangeQuery(fields, options) {
  ...
  Object.keys(options).forEach(key => {
    ...
    if (fields[key].isArray) {
      if (value[0] != null) range[`${key}.${value[0] - 1}`] = { $exists: true };
      if (value[1] != null) range[`${key}.${value[1]}`] = { $exists: false };
      return;
    }
...
function generateQuery(fields, options, not) {
  ...
  for (const key in options)
    if (Object.prototype.hasOwnProperty.call(options, key)) {
      switch (key) {
        ...
        case "match":
          if ("or" in options.match)
            for (const key in options.match.or) {
              if (
                Object.prototype.hasOwnProperty.call(options.match.org, key)
              ) {
                const q = {};
        ...
        case "exists":
          $and.push(
            mapValues(options.exists, (value, key) => {
              if (!(key in fields)) return void 0;

              if (fields[key].isArray) return value ? { $ne: [] } : [];
  ...
  if (not) $and = $and.map(applyNotOperator);
...
function deleteCollection(db, collectionPath, batchSize = 2000) {
